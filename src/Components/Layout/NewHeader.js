...
const NavListTItle = styled(Link)`
  ...
  color: ${({ $current, theme: { accentColor } }) =>
    $current ? accentColor : "#333"};
  &:hover {
    color: ${({ theme: { accentColor } }) => accentColor};
    border-bottom: 2px solid ${({ keyBlue }) => keyBlue};
    box-sizing: border-box;
  }
`;
          ...
          {navArr.map((item, index) => (
            <NavList
              ...
              onMouseOver={() => {
                ...
                return item.children.length > 0 && onHeaderOpen(item.path);
              }}
            ...
            >
              <NavListTItle
                ...
                to={{
                  pathname: item.path,
                  state: { pageState: item.pageState || "" },
                }}
              >
                {item.text}
              </NavListTItle>
              ...
              {(item.children && (
                <DepthUl
                  headerCurrent={depthCurrent === item.path ? true : false}
                >
                      ...
                      {item.children.map((cItem, cIndex) => (
                        <li key={`navChildren${cIndex}`}>
                          <Link
                            to={{
                              pathname: item.path,
                              state: { pageState: cItem.pageState },
                            }}
                          >
                            {cItem.text}
                          </Link>
