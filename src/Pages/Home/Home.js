...
const ShortcutList = styled.li`
  ...
  transform: ${({ scrl }) => `translateY(${scrl / 30}px)`};
  transition: ${({ delay }) => `all${delay}`};
`;
    ...
    <Container>
      ...
      <IntroTextWrap>
        ...
        <span className="middle_text">
          I'll consider everything you say and do my best
        </span>
        <span>클라이언트가 원하는 디자인을 위해 귀 기울이겠습니다.</span>
      </IntroTextWrap>
      <ShortCutWrap>
        <ShortcutList shortcutMove={shortcutMove} scrl={scrollY} delay=".3s">
          <Link to="/page_detail">
            <img src={ShortCut1} alt="shortcut img" />
            <span>Page Detail</span>
          </Link>
        </ShortcutList>
        <ShortcutList shortcutMove={shortcutMove} scrl={scrollY} delay=".7s">
          <Link to="/">
            <img src={ShortCut2} alt="shortcut img" />
            <span>Look Book</span>
          </Link>
        </ShortcutList>
        <ShortcutList shortcutMove={shortcutMove} scrl={scrollY} delay=".5s">
          <Link to="/">
